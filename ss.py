...
class SoundboardApp:
    ...
    def open_settings_window(self):
        ...
        tk.Checkbutton(win, text="Auto Normalize MP3 Volume", variable=norm_var).pack(anchor="w", pady=5, padx=10)
                ...
                if self.settings.get("normalize_volume"):
                    try:
                        ...
                        if loudness > -3.0:
                            ...
                            self.log_message(f"{filename} is quiet ({loudness:.1f} dB), boosting +5dB")
                            audio = audio + 5
    ...
    def start_hotkeys(self):
        ...
        for key, path in self.keybinds.items():
            keyboard.add_hotkey(key, lambda p=path: self.play_sound(p))
